...
function VSSynthretro(self) {
	...
	class VSSynthretro {
		constructor(args) {
			...
			this.methods = Object.getOwnPropertyNames(VSSynthretro.prototype);
			this._args = args || {};
		}
		setPlayer1(player1) {
		...
		send(apikey, options) {
			return request(self.BASE("vs-synthretro", apikey), options);
		}
		toBuffer(cb) {
